/**
 * The registered `knowledge.detail` entry component for search: the reader pane
 * shown in KnowledgeRoot's third column. It follows the selection held in the
 * package-local {@link useSelectedId} store (written by the workspace list), so
 * a click on a result opens the full document here without cross-plugin
 * messaging. When nothing is selected — or the `检索` row is not active — the
 * shell leaves this column empty via its `only: active` filter.
 * @module @deepseek-ai/dsh-client-ui-search/client/SearchDetail
 */
...
import css from './SearchDetail.module.css'

/** Composed props: runtime share plus the registrant-injected search face. */
export type SearchDetailProps = PropsRuntime<'knowledge.detail'> & InjectFace<SearchInjected>

/**
 * Render the search detail reader inside its inject-face provider.
 * @param props - composed slot props (runtime share + injected face).
 * @returns the search detail reader.
 */
export function SearchDetail(props: SearchDetailProps) {
  return (
